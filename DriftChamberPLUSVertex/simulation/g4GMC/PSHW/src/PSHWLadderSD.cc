...
namespace pshw {
  ...
  PSHWLadderSD::PSHWLadderSD(const G4String &name, const G4String& hitsCollectionName, const crd::SimpleConfig& config) :
...
//
//          if ( !geom->hasElement<PSHWtracker>() ) {
//                  throw cet::exception("GEOM")
//                  << "Expected I Trackers but found neither.\n";
//          }

    if ( !GeomService::Instance()->hasElement<PSHWtracker>() ) {
      exc::exceptionG4 e("GEOM","Fatal Error",0);
      e<< "Expected PSHW Trackers but found neither.\n";
      e.error();
    }
  }

  PSHWLadderSD::~PSHWLadderSD(){ }

  G4bool PSHWLadderSD::ProcessHits(G4Step* aStep, G4TouchableHistory* ){
...
//                                          << _currentSize << endl;
                          ...
                          e<< "PSHW: Maximum number of particles reached "
                              << SensitiveDetectorName
                              << ": "
                              << _currentSize << endl;
                          e.error();
...
//          string volName = aStep->GetTrack()->GetVolume()->GetName();
          int volCpNum = 0;
          _roSubShell = 0;
          Layer* tmpLayer = pshwtracker->getLayer(_layer);
...
//          cout<<"volCpNum "<<volCpNum<<" Sec "<<_phiSec<<" lad "<<_ladder<<endl;
          ...
          if ( verboseLevel>=2 )  std::cout<<"PSHW: G4 hit pos in World "<<prePosWorld[0]<<" "<<prePosWorld[1]<<" "<<prePosWorld[2]<<std::endl;
          G4ThreeVector prePosTracker = prePosWorld - _pshwDetCenter;
          G4ThreeVector postPosTracker = aStep->GetPostStepPoint()->GetPosition() - _pshwDetCenter;
          ...
          ROGeometryHandle *roghndl = pshwtracker->getROGeometryHandle();
