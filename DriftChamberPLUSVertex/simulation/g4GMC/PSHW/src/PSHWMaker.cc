...
void PSHWMaker::loadBarrelTracker( crd::SimpleConfig const& config ){
 ...
 char tmpVarName[50];
  ...
  for (int il=0; il<_nLayers; ++il) {
    ...
    if (_LayersInnerRad[il]>_rOut) { _rOut=(_LayersInnerRad[il]+_LaddersThickness[il])/cos(0.5*_phiAngles[il]); }
...
//        cout<< _ROSecondSideDim_fwd[il]<<" "<<_ROSecondSideInsul_fwd[il]<<endl;
        float lNRwPerOneDltRO = ( ( (_ROfirstSideDim_fwd[il]+_ROfirstSideInsul_fwd[il])/(2.0*tan( 0.5*_phiAngles_fwd[il]) ) ) / ( _ROSecondSideDim_fwd[il]+_ROSecondSideInsul_fwd[il] ) );
        int tmpNRwPerOneDltRO = rint(lNRwPerOneDltRO);
        int tmpNRwPerAddOneDltRO = 0;
        float tmpLeft = fabs(lNRwPerOneDltRO-tmpNRwPerOneDltRO);
        if (tmpLeft>0.0) { tmpNRwPerAddOneDltRO = rint( 1.0/tmpLeft ); }
...
//        cout<<nROSndSdperLad<<" "<<tmpNRwPerOneDltRO<<endl;
        for (unsigned int iSdRo=1; iSdRo<nROSndSdperLad; ++iSdRo) {
           if ( iSdRo%tmpNRwPerOneDltRO==0 ) { ++nROperRow; }
