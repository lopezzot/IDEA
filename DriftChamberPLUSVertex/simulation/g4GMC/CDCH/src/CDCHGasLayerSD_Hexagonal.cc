...
namespace cdch {
  ...
  CDCHGasLayerSD_Hexagonal::CDCHGasLayerSD_Hexagonal(const G4String &name, const G4String& hitsCollectionName, const crd::SimpleConfig& config) :
      CDCHGasLayerSD( name, hitsCollectionName, config) { }


  CDCHGasLayerSD_Hexagonal::~CDCHGasLayerSD_Hexagonal() { }

  G4bool CDCHGasLayerSD_Hexagonal::ProcessHits(G4Step* aStep, G4TouchableHistory*){
    ++_currentSize;
...
//                                    << _currentSize << endl;
    ...
    try {
            ...
            if ( verboseLevel>=2 )  std::cout<<"hit 0 wire dist " <<sqrt(sum_of_squares(pos[0]-xywire[0], pos[1]-xywire[1], pos[2]-xywire[2])) <<std::endl;
            ...
            switch (wire%3) {
                    case 1:
                            ...
                            if(ring==0 || ring==maxR) {
                                    ...
                                    ++ring1;
                                    wire1=wire;
                                    cdchtracker->getCellGeometryHandle()->SelectCell(_superlayer,ring1,wire1);
                    ...
                    case 2:
                            ...
                            if (ring==0 || ring==maxR) {
                                    ...
                                    cdchtracker->getCellGeometryHandle()->SelectCell(_superlayer,ring1,wire/*wire1*/);
                                    tdist=cdchtracker->getCellGeometryHandle()->DistFromWireCenter(hit);
                                    if (mdist>tdist) {
                                            mdist=tdist;
                                            ring=ring1;
