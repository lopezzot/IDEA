...
namespace svx {
  ...
  SVXLadderSD::SVXLadderSD(const G4String &name, const G4String& hitsCollectionName, const crd::SimpleConfig& config) :
...
//
//          if ( !geom->hasElement<SVXtracker>() ) {
//                  throw cet::exception("GEOM")
//                  << "Expected I Trackers but found neither.\n";
//          }

    if ( !GeomService::Instance()->hasElement<SVXtracker>() ) {
      exc::exceptionG4 e("GEOM","Fatal Error",0);
      e<< "Expected SVX Trackers but found neither.\n";
      e.error();
    }
  }

  SVXLadderSD::~SVXLadderSD(){ }

  G4bool SVXLadderSD::ProcessHits(G4Step* aStep, G4TouchableHistory* ){
...
//                                          << _currentSize << endl;
                          ...
                          e<< "SVX: Maximum number of particles reached "
                              << SensitiveDetectorName
                              << ": "
                              << _currentSize << endl;
                          e.error();
...
//          string volName = aStep->GetTrack()->GetVolume()->GetName();
          int volCpNum = 0;
          _roSubShell = 0;
          Layer* tmpLayer = svxtracker->getLayer(_layer);
...
//          if (isFw) {
//          cout<<"Hit in vol "<<volName<<" lyStr "<<volName.substr(10,3)<<" Layer "<<_layer<<" subLadder "<<_roSubShell<<endl;
...
//          }
          ...
          if ( verboseLevel>=2 )  std::cout<<"SVX: G4 hit pos in World "<<prePosWorld[0]<<" "<<prePosWorld[1]<<" "<<prePosWorld[2]<<std::endl;
          G4ThreeVector prePosTracker = prePosWorld - _svxDetCenter;
          G4ThreeVector postPosTracker = aStep->GetPostStepPoint()->GetPosition() - _svxDetCenter;
          ...
          ROGeometryHandle *roghndl = svxtracker->getROGeometryHandle();
...
//            cout<<" halfLength "<<roghndl->GetLadder()->getDetail()->halfLength()<<endl;
          ...
          if (isFw) {
            prePosLoc[0] -= roghndl->GetLadder()->getDetail()->width()*0.5;
            prePosLoc[2] += roghndl->GetLadder()->getDetail()->halfLength();
            prePosLoc[0] *= -1.0;
//            prePosLoc[2] *= -1.0;
          } else {
...
//          if (isFw) {
//          cout<<"prePosLoc "<<prePosLoc[0]<<" CahnFstSdDim "<<(roghndl->GetROChanFstSdDim()+roghndl->GetROChannel()->getFstSideInsul())<<endl;
